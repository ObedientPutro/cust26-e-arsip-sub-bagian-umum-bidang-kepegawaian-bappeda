...
const performDelete = () => {
    if (deleteLetter.value) {
        deleteForm.delete(route('outgoingLetter.delete', deleteLetter.value.id), {
            onSuccess: () => {
                closeModal();
            },
        });
...
const headers = [
    ...
    { key: 'subject', label: 'Subjek', class: 'text-center' },
    { key: 'recipient', label: 'Penerima', class: 'text-center' },
...
<template>
    ...
    <div class="bg-base-100 overflow-hidden shadow-md rounded-lg p-6">
        <DataTable :headers="headers" :items="letters" :addUrl="route('outgoingLetter.new')">
            <!-- Mendefinisikan Judul & Deskripsi Header -->
            <template #header-info>
                <h2 class="card-title text-xl font-bold">Surat Keluar</h2>
                <p class="text-sm text-base-content/70">Kelola data seluruh surat keluar.</p>
            </template>
                ...
                <div class="card-body p-4">
                    <div class="flex justify-between items-start gap-4">
                        ...
                        <div class="flex gap-1">
                            <a :href="route('outgoingLetter.modify', item.id)" class="btn btn-square btn-ghost btn-sm">
                    ...
                    <dl class="text-xs space-y-1">
                        ...
                        <div class="flex justify-between"><dt class="opacity-70">Penerima:</dt><dd class="font-medium">{{ item.recipient }}</dd></div>
                        <div class="flex justify-between"><dt class="opacity-70">Tanggal:</dt><dd class="font-medium">{{ formatDate(item.letter_date) }}</dd></div>
                    </dl>
                </div>
            </template>
        </DataTable>
